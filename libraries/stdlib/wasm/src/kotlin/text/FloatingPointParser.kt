...
internal fun parseDouble(string: String): Double {
    ...
    parseUnsignificants()
    val signed = parseSign()

    if (tryParseWord("NaN")) {
        parseUnsignificants()
        if (index != string.length) numberFormatError(string)
        return if (!signed) Double.NaN else -Double.NaN
    }
    if (tryParseWord("Infinity")) {
        parseUnsignificants()
        if (index != string.length) numberFormatError(string)
        return if (!signed) Double.POSITIVE_INFINITY else -Double.NEGATIVE_INFINITY
    }
    ...
    return numberToDouble(signed, scale, number)
}