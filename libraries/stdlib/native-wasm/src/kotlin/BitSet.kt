 ...
 */
public open class BitSetImpl internal constructor(size: Int = ELEMENT_SIZE) {

    @kotlin.native.internal.CanBePrecreated
    companion object {
        // Default size of one element in the array used to store bits.
        internal const val ELEMENT_SIZE = 64
    ...
    /** True if this BitSetImpl contains no bits set to true. */
    val isEmpty: Boolean
    ...
    /** Sets all bits in the BitSetImpl to `false`. */
    fun clear() {
    ...
    private inline fun doOperation(another: BitSetImpl, operation: Long.(Long) -> Long) {
    ...
    /** Performs a logical and operation over corresponding bits of this and [another] BitSetImpls. The result is saved in this BitSetImpl. */
    fun and(another: BitSetImpl) = doOperation(another, Long::and)

    /** Performs a logical or operation over corresponding bits of this and [another] BitSetImpls. The result is saved in this BitSetImpl. */
    fun or(another: BitSetImpl) = doOperation(another, Long::or)

    /** Performs a logical xor operation over corresponding bits of this and [another] BitSetImpls. The result is saved in this BitSetImpl. */
    fun xor(another: BitSetImpl) = doOperation(another, Long::xor)

    /** Performs a logical and + not operations over corresponding bits of this and [another] BitSetImpls. The result is saved in this BitSetImpl. */
    fun andNot(another: BitSetImpl) {
        ensureCapacity(another.lastIndex)
        var index = 0
        while (index < another.bits.size) {
    ...
    /** Returns true if the specified BitSetImpl has any bits set to true that are also set to true in this BitSetImpl. */
    fun intersects(another: BitSetImpl): Boolean =
